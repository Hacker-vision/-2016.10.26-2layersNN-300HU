%load('ex4data1.mat'); %5000training samples
...
%m = size(X, 1);%size(X,1)行 size(X,2)列

% Randomly select 100 data points to display 对随机100个样本进行可视化
sel = randperm(size(X, 1));%random(n): generate a random sequence using 1- n 随机打乱一个数字序列
sel = sel(1:100);

displayData(X(sel, :)); %图像、灰度的一些知识？？？？？？
